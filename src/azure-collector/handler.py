...
S3_KEY = os.environ.get('S3_KEY', 'data/services-azure.json')
STATISTICS_KEY = os.environ.get('STATISTICS_KEY', 'data/statistics-azure.json')
CHANGELOG_KEY = os.environ.get('CHANGELOG_KEY', 'data/changelog-azure.json')
...
def lambda_handler(event, context):
    ...
    print(f"[azure-changelog] {new_changes} new changes detected")

    # Persist to DynamoDB
    if new_changes > 0:
        new_entries = changelog[:new_changes]
        dynamo_written = write_changes("azure", new_entries)
        print(f"[dynamo] {dynamo_written} changes written")
    update_service_data("azure", services)
    ...
    print(f"[azure-statistics] {stats['summary']['totalServices']} services")
    ...
    _invalidate(['/data/services-azure.json', '/data/changelog-azure.json', '/data/statistics-azure.json'])
    return {"statusCode": 200, "body": f"Wrote {len(services)} Azure services"}
